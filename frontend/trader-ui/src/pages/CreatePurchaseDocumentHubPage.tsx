...
import { PURCHASE_INVOICE_TYPES, type InvoiceTypeConfig } from '../lib/invoiceTypes';
import { PageHeader, LoadingBlock } from '../components/ui';

export default function CreatePurchaseDocumentHubPage() {
  const navigate = useNavigate();
  const [documents, setDocuments] = useState<InvoiceTypeConfig[]>(PURCHASE_INVOICE_TYPES);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const load = async () => {
      try {
        const res = await purchasesApi.getDocumentCatalog();
        const rows = res.data.message?.documents;
        if (Array.isArray(rows) && rows.length > 0) {
          setDocuments(rows);
        }
      } catch {
        setDocuments(PURCHASE_INVOICE_TYPES);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Create Purchase Document"
        description="Tax invoice, commercial bill, bill of supply, or debit note for supplier purchases."
        actions={
          <button
            type="button"
