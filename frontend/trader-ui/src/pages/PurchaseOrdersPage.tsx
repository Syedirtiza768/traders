...
import { FileText, Plus, ShoppingCart, TrendingDown } from 'lucide-react';
...
const STATUS_TABS = ['All', 'Draft', 'To Receive and Bill', 'To Bill', 'Completed', 'Cancelled'];
const PAGE_SIZE = 15;

function buildPurchaseInvoicePrefill(order: any, listSearch?: string) {
  ...
  params.set('sourceType', 'purchase-order');
  if (listSearch) params.set('list', listSearch);

  const query = params.toString();
  return `/purchases/new${query ? `?${query}` : ''}`;
}
...
export default function PurchaseOrdersPage() {
  ...
  const buildDetailPath = (orderName: string) => {
    ...
    return `/purchases/orders/${encodeURIComponent(orderName)}${query}`;
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <PageHeader
        ...
        description="Manage supplier commitments before receipt, billing, and payment."
        actions={
          <button type="button" onClick={() => navigate('/purchases/orders/new')} className="btn-primary flex items-center gap-2">
      ...
      {workflow === 'unpaid-invoices' && (
        <WorkflowFilterBanner
          ...
          description="Showing purchase orders that still have unpaid downstream invoices."
      ...
      {workflow === 'awaiting-invoice' && (
        <WorkflowFilterBanner
          title="Workflow filter active"
          description="Showing submitted purchase orders that have not yet produced any purchase invoice."
          onClear={() => updateSearchParams({ workflow: null, page: null })}
        />
      )}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">
        ...
        <StatCard icon={TrendingDown} label="Visible Order Value" display={formatCompact(visibleValue)} color="green" />
      ...
      <div className="hidden md:block table-container">
        <table className="data-table w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={6}><LoadingBlock compact label="Loading purchase orders…" /></td></tr>
              ...
              <tr>
                <td colSpan={6}>
                  <EmptyState
                    ...
                    action={
                      <button type="button" className="btn-primary" onClick={() => navigate('/purchases/orders/new')}>
                        New Purchase Order
                      </button>
                    }
                  />
              ...
              filteredOrders.map((order) => (
                ...
                >
                  ...
                  <td className="text-gray-700 dark:text-slate-300">{order.supplier_name || order.supplier || '—'}</td>
                      ...
                      {order.supplier ? (
                        <>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(buildPurchaseInvoicePrefill(order, listSearch));
                            }}
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(appendPreservedListQuery(`/suppliers/${encodeURIComponent(order.supplier)}`, listSearch));
                            }}
      ...
      <div className="md:hidden card divide-y divide-gray-100 dark:divide-slate-800">
        {loading ? (
          <LoadingBlock compact label="Loading purchase orders…" />
        ) : filteredOrders.length === 0 ? (
          <EmptyState compact title="No purchase orders found" description={search || status !== 'All' || workflow ? 'Try adjusting filters or search.' : undefined} />
        ) : (
          filteredOrders.map((order) => (
