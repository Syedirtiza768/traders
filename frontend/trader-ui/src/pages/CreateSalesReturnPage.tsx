...
export default function CreateSalesReturnPage() {
  ...
  useEffect(() => {
    ...
    if (linesParam) {
      try {
        ...
        console.error('Failed to parse sales return line prefills:', err);
      }
    }
  }, [searchParams]);
  ...
  const readinessChecks = useMemo(
    ...
    [customer, invoiceName, validLineCount],
  );
  ...
  const handleSubmit = async () => {
    if (!customer) {
      setError('Please select a customer before creating the return.');
      return;
    }
    if (!invoiceName) {
      setError('A source sales invoice is required for return creation.');
      return;
    }
    ...
    setError(null);
    try {
      const response = await salesApi.createReturnInvoice({
        customer,
      ...
      setError(err?.response?.data?.exception || 'Could not create sales return.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-gray-500">Create a credit-note style sales return against a submitted sales invoice.</p>
        </div>
      ...
      <div className="rounded-lg border border-brand-200 bg-brand-50 px-4 py-3 text-sm text-brand-900">
        ...
        <div className="mt-1 text-brand-800">{invoiceName ? `Returning against invoice ${invoiceName}.` : 'Choose the source invoice from a sales invoice action.'}</div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="card p-6 lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <Field label="Return Against Invoice">
              <input value={invoiceName} onChange={(e) => setInvoiceName(e.target.value)} className="input-field" placeholder="Sales Invoice" />
            </Field>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="rounded-lg bg-gray-900 px-4 py-3 text-white">
            <div className="text-xs uppercase tracking-wide text-gray-300">Estimated Credit Amount</div>
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-3 text-xs text-blue-800">
            ...
            This flow creates a draft Sales Invoice marked as a return and links it to the source invoice using <code>return_against</code>.
          </div>
        </div>
      </div>
...
function ReadinessCard({ ready, issues }: { ready: boolean; issues: string[] }) {
  return (
    <div className={`rounded-lg border px-4 py-3 text-sm ${ready ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
      ...
      {ready ? (
        <p className="mt-1 text-xs text-emerald-700">Customer, source invoice, and return line checks are satisfied.</p>
      ) : (
